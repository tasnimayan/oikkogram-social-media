...
import { sendFriendRequest } from '@/utils/queries';
...
const AddFriendButton = ({ friendId }:{ friendId:string}) => {
  ...
  return <button onClick={handleAddFriend} className='border rounded px-2 py-1 bg-blue-400'>{status?? 'Add Friend'}</button>;
};
